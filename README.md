...
- Поиск компаний по категории работает только через не-AI источники: CSV, CEIDG API, официальный Google Places API, а также точечный веб-поиск по бизнес-каталогам и реестрам (источник `registries`, см. ниже).
...
### Источник "Реестры" (`registries`): Польша и Украина

Помимо Google Places и CEIDG API, источник `registries` делает точечный поиск через обычный веб-поиск (Bing `site:` запросы вида `[категория] [город] site:домен`), без массового обхода реестров целиком. Найденная страница компании открывается точечно только для того, чтобы вытащить контакты/сайт.

Польша (`discoverCompaniesFromPublicRegistriesPoland` в `server.js`):
- CEIDG (`aplikacja.ceidg.gov.pl`, `biznes.gov.pl` — wyszukiwarka firm)
- Panorama Firm (`panoramafirm.pl`)
- Aleo.com (NIP/KRS/REGON)
- PKT.pl
- Regon24.pl
- rejestr.io

Украина (`discoverCompaniesFromPublicRegistriesUkraine` в `server.js`) — **без прямого массового обращения к реестру ЄДР**:
- точечное открытие уже найденных карточек на opendatabot.ua и youcontrol.com.ua (не обход по кодам ЄДРПОУ)
- обычный веб-поиск по локальным украинским бизнес-каталогам (по аналогии с Panorama Firm)

Если для Украины понадобится больше объёма и точности данных, стоит рассмотреть **платный API Opendatabot или YouControl** (легальная агрегация данных ЄДР по соглашению с Минюстом) — сейчас это не реализовано, так как нет API-ключа; текущая реализация — только точечный веб-поиск.

Найденные через `registries` компании сверяются с уже найденными через Google Places по NIP/REGON/EDRPOU, телефону, домену сайта, Google Maps ссылке или названию+адресу (`buildCompanyKeys` в `store.js`) — при совпадении данные (NIP/KRS/REGON/EDRPOU, доп. контакты) дополняют существующую карточку лида вместо создания дубля.
...
Если нужно поднять сервер и сразу прогнать smoke-проверку одной командой:
